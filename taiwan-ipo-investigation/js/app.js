...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function initDarkMode() {
    ...
    // 如果用戶之前沒有設置過模式，且瀏覽器偏好暗模式，則自動設置為暗模式
    if (!localStorage.getItem('darkMode') && prefersDarkMode) {
      body.classList.add('dark-mode');
      if (darkModeToggle) {
        darkModeToggle.innerHTML = '<i class="fas fa-sun"></i>';
      }
  ...
  function initSecretFeature() {
    if (!secretTrigger || !secretContent || !secretClose || !secretUnderstand) return;
  ...
  function initShareFeature() {
    ...
    document.querySelectorAll('.share-link').forEach(link => {
      link.addEventListener('click', function(e) {
        ...
        const platform = this.querySelector('span')?.textContent.trim().toLowerCase() || '';
        const url = encodeURIComponent(window.location.href);
        const text = encodeURIComponent('台灣證券業IPO與ETF募集制度黑幕揭密 - #終結IPO制度暴力');
        let shareUrl = '';
