...
function initProductTableView() {
  ...
  if (!productContainer) {
    ...
    if (classicTable && classicTable.parentNode) {
      classicTable.parentNode.insertBefore(productContainer, classicTable.nextSibling);
    }
  }
...
function generateProductTableView(data, container) {
  ...
  products.forEach(product => {
    ...
    brokers.forEach(broker => {
      const match = data.find(item => item.product === product && item.broker === broker);
      if (match) {
  ...
  const grandAverage = brokerProducts > 0 ? Math.round(grandTotal / brokerProducts) : 0;
...
function setupViewToggle() {
  ...
  if (!classicViewBtn || !productViewBtn || !classicTable || !productTable) return;
...
function createDataStats() {
  ...
  const brokerCount = new Set(data.map(item => item.broker)).size;
  const productCount = new Set(data.map(item => item.product)).size;
  const avgAmount = Math.round(totalAmount / data.length);
  
  // 創建統計卡片
  const statsCard = document.createElement('div');
  statsCard.className = 'data-stats';
  statsCard.innerHTML = `
    <div class="stat-item">
      <div class="stat-value">${totalAmount.toLocaleString()}</div>
      <div class="stat-label">總責任額(萬元)</div>
    </div>
    <div class="stat-item">
      <div class="stat-value">${brokerCount}</div>
      <div class="stat-label">券商數量</div>
    </div>
    <div class="stat-item">
      <div class="stat-value">${productCount}</div>
      <div class="stat-label">產品數量</div>
    </div>
    <div class="stat-item">
      <div class="stat-value">${avgAmount.toLocaleString()}</div>
      <div class="stat-label">平均責任額(萬元)</div>
    </div>
  `;
  
  // 獲取表格容器並添加統計卡片
  const tableContainer = document.querySelector('.table-wrapper').parentNode;
  ...
  tableContainer.insertBefore(statsCard, document.querySelector('.table-info'));
...
function setupResizeHandle() {
  const productTable = document.getElementById('product-table-container');
  const resizeHandle = productTable ? productTable.querySelector('.resize-handle') : null;
  ...
  resizeHandle.addEventListener('mousedown', (e) => {
    startY = e.clientY;
    startHeight = parseInt(document.defaultView.getComputedStyle(productTable).height, 10);
...
function downloadExcel(data, isProductView) {
  ...
  if (isProductView) {
    ...
    // 下載CSV文件 (Excel可以打開)
    downloadFile(csvContent, 'ETF責任額數據_產品視圖.xlsx', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  } else {
    // 經典視圖格式
    ...
    // 下載CSV文件 (Excel可以打開)
    downloadFile(csvContent, 'ETF責任額數據_經典視圖.xlsx', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  }
}
