...
// 全局變數
let csvData = [];
const branchCountsMap = {}; // 新增：用於儲存券商分點數量
let dataTable = null;
...
function initProductTable(data) {
  ...
  const sortedProductObjects = Object.values(productDetails).map(p => {
  ...
  // 計算平均責任額的閾值，用於高亮顯示
  const averages = sortedProductObjects.map(p => p.average).filter(a => a > 0);
...
function generateKeyInsights() {
  ...
  const maxGap = Math.max(
    ...Object.values(productMap).map(arr => Math.max(...arr) - Math.min(...arr))
  );
