...
class HarassmentVoicesModule {
  constructor() {
    ...
    this.simulatorDialogues = [
      ...
      },
      {
        category: "人格侮辱",
        message:
          "「妳裙子穿得比別人短，責任額跟其他男生一樣不覺得愧疚嗎」、「你是女生要會利用自己優勢」、「女生怎麼可能賣不出去是妳要不要而已你知道方法的」",
        compliantConsequence:
          "配合後果：\n• 忍受性騷擾言論繼續工作\n• 被迫穿著暴露服裝見客戶\n• 自尊心受到嚴重創傷\n• 客戶開始對你有不當想法\n• 身心俱疲，開始看心理醫生",
        resistConsequence:
          "拒絕後果：\n• 主管惱羞成怒：「妳要告就去告啊！」\n• 被調到最偏遠的分公司\n• 工作內容變成打雜跑腿\n• 升遷機會永遠與你無關\n• 被同事排擠孤立",
  ...
  initPagination() {
    ...
    // 初始載入時更新分頁按鈕狀態
    this.updatePaginationButtons(this.getFilteredData().length);
  }
      ...
      if (currentPageData.length === 0) {
        container.innerHTML = `
          <div class="text-center py-12">
            ...
            <p class="text-neutral-500">目前沒有符合條件的心聲案例</p>
          </div>
        `;
        return;
      }

      currentPageData.forEach((caseItem, index) => {
  ...
  updatePaginationButtons(totalItems) {
    ...
    const updateButtons = (prevButton, nextButton, pageInfoElement) => {
      ...
      if (this.currentPage <= 1) {
        ...
        prevButton.classList.remove("hover:bg-neutral-200");
      } else {
        prevButton.disabled = false;
        prevButton.classList.remove("opacity-50", "cursor-not-allowed");
        prevButton.classList.add("hover:bg-neutral-200");
      }
      ...
      if (this.currentPage >= totalPages) {
        ...
        nextButton.classList.remove("hover:bg-neutral-200");
      } else {
    ...
    // 桌面版和手機版統一應用
    updateButtons(prevButton, nextButton, pageInfo);
    updateButtons(prevButtonMobile, nextButtonMobile, pageInfoMobile);
  }
