...
function generateKeyInsights() {
  const dt =
    $.fn.dataTable && $("#csv-table").DataTable
      ? $("#csv-table").DataTable()
      : null;
  if (!dt) return;

  const visible = dt.rows({ filter: "applied" }).data().toArray();
  ...
  const toDate = (s) => {
    ...
    return new Date(2000 + y - 1911, m - 1, d);
  };
  const daysIn = (p) => {
    const [s, e] = p.split("-");
    const sD = toDate(s);
    const eD = toDate(e.includes("/") ? e : `${s.split("/")[0]}/${e}`);
    return (eD - sD) / 864e5 + 1;
  };
  const monthKey = (p) => p.slice(0, 6); // 113/03
  ...
  const rows = visible.map((r) => ({
    ...
    period: r[3],
    days: daysIn(r[3]),
    dateStart: toDate(r[3].split("-")[0]),
  }));

  /* ---- 壓力指標計算 ---- */
  const maxResp = Math.max(...rows.map((r) => r.amt));
  const avgResp = sum(rows.map((r) => r.amt)) / rows.length;
  const ratio = (maxResp / avgResp).toFixed(1);

  const avgDays = (sum(rows.map((r) => r.days)) / rows.length).toFixed(1);
  const minDays = Math.min(...rows.map((r) => r.days));
  const cutPct = ((1 - avgDays / 30) * 100).toFixed(0);

  const totalAmt = sum(rows.map((r) => r.amt));
  const totalDays = sum(rows.map((r) => r.days));
  const perDay = totalAmt / totalDays;
  ...
  const [hellMonth, hellCount] = Object.entries(monthMap).sort(
    (a, b) => b[1] - a[1]
  )[0]; // 最大值
  ...
  rows.forEach((r) => {
    ...
    dayMap[key] = (dayMap[key] || 0) + r.amt;
  });
  const [peakDateISO, peakAmt] = Object.entries(dayMap).sort(
    (a, b) => b[1] - a[1]
  )[0];
  const peakDateROC = (() => {
    // 轉回 ROC
    const d = new Date(peakDateISO);
    return `${d.getFullYear() - 1911}/${(d.getMonth() + 1)
      .toString()
  ...
  const assign = (id, val) => {
    ...
    if (el) {
      el.textContent = fmt(Number(val));
      el.dataset.target = val; // ⇦ 供動畫取值
      el.dataset.animated = ""; // 讓動畫可重播
    }
  };

  assign("ins-maxResp", maxResp);
  assign("ins-avgResp", avgResp);
  assign("ins-ratioResp", ratio);
  assign("ins-avgDays", avgDays);
  assign("ins-minDays", minDays);
  assign("ins-cutPct", cutPct);
  assign("ins-avgPerDay", perDay.toFixed(1));
  assign("ins-hrQuota", perHour.toFixed(2));
  assign("ins-hellMonth", hellMonth.split("/")[1]); // 03 → 3月
  assign("ins-hellCount", hellCount);
  assign("ins-peakAmt", peakAmt);
  const peakDateEl = document.getElementById("ins-peakDate");
  if (peakDateEl) peakDateEl.textContent = peakDateROC;

  animateInsightNumbers(); // 撥放動畫
}
...
function animateInsightNumbers() {
  ...
  if (!panel) return;
  const nums = panel.querySelectorAll(".counter");
  ...
  const run = (el) => {
    ...
    const tick = (now) => {
      ...
      el.textContent = (target * p).toLocaleString(undefined, {
        maximumFractionDigits: 2,
      });
      if (p < 1) requestAnimationFrame(tick);
      else el.dataset.animated = "done";
    };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const dt =
    $.fn.dataTable && $("#csv-table").DataTable
      ? $("#csv-table").DataTable()
      : null;
  if (dt) $("#csv-table").on("draw.dt", generateKeyInsights);
});
