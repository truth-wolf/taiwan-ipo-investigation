...
function initClassicTable(data) {
  ...
  const columns = [
    { title: "券商" },
    { title: "產品", render: function(data, type, row) {
...
function prepareProductTableContainer() {
  ...
  // 確認表格容器存在
  const tableWrapper = document.querySelector('.table-wrapper') || document.querySelector('.overflow-x-auto');
...
function initViewButtons() {
  ...
  setTimeout(() => {
    ...
    if (classicViewBtn && productViewBtn) {
      classicViewBtn.addEventListener('click', () => toggleTableView('classic'));
      productViewBtn.addEventListener('click', () => toggleTableView('product'));
    }
  }, 500);
}
...
function initProductTable(data) {
  ...
  products.forEach(product => {
    ...
    brokers.forEach(broker => {
      const match = data.find(item => item[1] === product && item[0] === broker);
      if (match) {
...
function addTotalInformation(data) {
  ...
  infoElement.innerHTML = `
    <div class="flex flex-col md:flex-row md:justify-between text-neutral-600">
      ...
      <span>券商數量: <strong class="text-primary-dark">${uniqueBrokers.length}</strong></span>
      <span>產品數: <strong class="text-primary-dark">${uniqueProducts.length}</strong></span>
    </div>
  `;
...
function initDataVisualizations(data) {
  ...
  visualizationContainer.className = 'data-visualization grid grid-cols-1 md:grid-cols-2 gap-8 mt-8';
  
  // 添加到DOM中
  const tableInfo = document.querySelector('.table-info');
  if (tableInfo) {
    tableInfo.parentNode.insertBefore(visualizationContainer, tableInfo.nextSibling);
...
function createBrokerComparisonChart(data, container) {
  ...
  // 轉換為Chart.js格式
  const labels = Object.keys(brokerData).sort((a, b) => brokerData[b] - brokerData[a]);
        ...
        tooltip: {
          callbacks: {
            label: function(context) {
              return `${context.dataset.label}: ${context.raw.toLocaleString()} 萬元`;
            }
          }
        }
      },
  ...
  data.forEach(item => {
    ...
    if (periodMatch) {
      ...
      if (!isNaN(amount)) {
        ...
        timeData[dateKey].maxAmount = Math.max(timeData[dateKey].maxAmount, amount);
      }
    }
  });
...
function setupEventListeners() {
  ...
  window.addEventListener('resize', function() {
    ...
    if (window.innerWidth >= 768 && !productTableInitialized && csvData.length > 0) {
      prepareProductTableContainer();
      initViewButtons();
    }
  });
